...
public class WebServer {
  ...
  public static final String GAME_SIGN_IN = "/signin";
  public static final String GAME_SIGN_OUT = "/signout";
  public static final String GAME_ROOM = "/game";
   ...
   */
  public WebServer(final TemplateEngine templateEngine, final Gson gson, final CenterGame centerGame ) {
  ...
  public void initialize() {
    ...
    //constructing the CenterGame object


    // Shows the Checkers game Home page.
    get(HOME_URL, new GetHomeRoute(templateEngine, centerGame));

    get(GAME_SIGN_IN, new GetSignInRoute(templateEngine));

    get(HELP, new GetHelpRoute(templateEngine));

    //get for GetGame
    get(GAME_ROOM, new GetGameRoute(templateEngine,centerGame));

    //post for SignIn
    post(HOME_URL, new PostSignInRoute(templateEngine , centerGame ));


    //post for Backup
    post(BACK_UP, new PostBackupMoveRoute(centerGame ));


    post(VALIDATE_MOVE, new PostValidateMove(templateEngine,centerGame) );

    //post for submitting a turn
    post(SUBMIT_TURN, new PostSubmitTurn(templateEngine , centerGame ));

    //post for submitting a turn
    post(CHECK_TURN, new PostCheckTurn(templateEngine , centerGame ));

    get(GAME_SIGN_OUT, new GetSignOutRoute(templateEngine, centerGame));

    //post for resigning game
    post(RESIGN_GAME, new PostResignGameRoute(centerGame));
